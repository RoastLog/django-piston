    ...
    def __getattr__(self, attr):
        ...
        class HttpResponseWrapper(HttpResponse):
            ...
            def _set_content(self, content):
                """
                Set the _container and _base_content_is_iter properties based
                on the type of the value parameter. This logic is in the
                construtor for HttpResponse, but doesn't get repeated when
                ...
                if not isinstance(content, basestring) and hasattr(content, '__iter__'):
                    ...
                    self._use_emitter = True
                else:
                    self._container = [content]
                    self._use_emitter = False
                # Django 1.7 and above does not have any custom internals to
                # define the type of the content
                if django.VERSION < (1, 7):
                    self._base_content_is_iter = self._use_emitter
